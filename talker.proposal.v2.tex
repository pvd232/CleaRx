...
\large Implementation Proposal Outline --- Revision 2}
...
\subsection{Freeze cross-runtime parity fixtures}
\begin{itemize}
    \item Capture the reference PyTorch outputs before implementing the speech-output graphs. Store fixtures for Thinker layer-24 hidden states and token embeddings; Talker router scores, selected experts, layer outputs, primary-code logits, and sampled primary code; all 15 residual MTP codes; and decoded PCM for fixed codec sequences.
    \item Use the \code{commons-jfk-inaugural} corpus item for the Thinker long-context fixture. Before capture, freeze the decode and resampling path, exact PCM bytes, sample format, channel count, sample rate, sample count, SHA-256 digest, and resulting model-input dimensions. The source OGG digest and duration identify the recording but do not determine those decoded bytes.
    \item Use one exact \code{int64} Code2Wav tensor with shape \code{[1,16,601]}; freeze all code values and the tensor's SHA-256 digest. With the \href{https://github.com/huggingface/transformers/blob/7d9754a05193eb79b1d86aa744b622b8068008cd/src/transformers/models/qwen3_omni_moe/modeling_qwen3_omni_moe.py\#L3505-L3515}{pinned 300-frame chunk and 25-frame left-context implementation}, the tensor exercises the first slice \code{0:300}, the steady context-inclusive slice \code{275:600}, and the final partial context-inclusive slice \code{575:601}.
    \item Record model revision, preprocessing inputs, generation parameters, tensor names, shapes, data types, tolerances, and checksums with every fixture so that a passing comparison identifies one reproducible reference execution.
    \item Port and verify MTP and Code2Wav independently against these fixtures before connecting either component to the Talker. Join the complete Thinker--Talker--MTP--Code2Wav chain only after each boundary passes its frozen-input comparison.
    \item Do not reduce the implementation schedule or declare the native waveform path complete until the frozen-input chain passes end to end.
\end{itemize}
...
\begin{itemize}
    \item Convert and package the Q8\_0 Talker, reference-precision MTP and Code2Wav tensors, speaker/voice-conditioning embeddings, and all metadata required for streaming generation.
    \item Reuse existing GGUF metadata keys and tensor names when they represent the same operation. Add Qwen3-Omni-specific names only for tensors or state that existing formats cannot represent.
    \item Verify tensor names, dimensions, strides, data types, checksums, and correspondence with the reference PyTorch weights.
    \item Preserve the reference weights for norms, routers, embeddings, and sensitive output heads; select each native storage and compute type from backend support and parity measurements.
\end{itemize}
...
\begin{itemize}
    ...
    \item Start from the existing \code{llama.cpp} residual-code predictor graph where its operations and tensor semantics match Qwen3-Omni. Treat this as graph reuse, not proof of model compatibility.
    ...
    \item Require frozen-logit parity at each residual step, then validate all 15 residual-code predictions and intermediate MTP hidden states against the reference implementation.
    \item Reuse one MTP scratch allocation across residual-codebook substeps.
\end{itemize}

\subsection{Port streaming Code2Wav}
\begin{itemize}
    \item Reconstruct the complete Qwen3-Omni causal Code2Wav graph with GGML CPU operators, adding kernels only where profiling demonstrates a gap.
    \item Reuse existing decoder operations and streaming-state machinery only when their dimensions, tensor semantics, padding, and state transitions match. Do not assume that the existing Qwen3-TTS codebook front end or decoder graph is architecture-compatible with Qwen3-Omni.
...
\subsection{Select native types from backend support and parity}
\begin{itemize}
    \item Thinker: Q4\_K\_M.
    \item AuT/projector: Q8\_0, with sensitive tensors at higher precision where required.
    \item Talker: Q8\_0.
    \item MTP and Code2Wav: BF16 defines the reference weights. Select native storage and compute types per tensor from backend support and frozen-fixture parity; permit F16 or F32 where GGML operators do not support BF16 or parity requires higher precision.
    \item Thinker and Talker KV: Q8\_0.
    \item Norms, routers, embeddings, and sensitive heads: preserve reference precision where required by parity; do not assume one native type across all operators.
    \item Vision tensors: absent.
    \item Recalculate module sizes, shared-arena requirements, and peak L4 memory after the actual per-tensor storage and compute types are fixed.
\end{itemize}
...
\begin{itemize}
    ...
    \item During speaking, release AuT and Whisper and use the arena for the Talker residency or offload buffers selected by the measured decision in Section~3.
...
\section{Measured Q8 Talker residency and conditional expert paging}

\subsection{Select the least complex residency mode that fits}
\begin{itemize}
    \item First test whether the exact Q8\_0 Talker can remain fully GPU-resident during speaking after AuT and Whisper weights and workspaces have been released.
    \item If full residency exceeds the measured envelope, benchmark ordinary \code{llama.cpp} CPU/GPU layer and tensor offload before introducing a new paging subsystem.
    \item Collect L4 peak-memory, routing, PCIe-transfer, and generation-latency traces for both modes under the final streaming workload.
    \item Implement per-expert paging only if neither full residency nor ordinary offload satisfies the memory and latency gates.
    \item Preserve exact Q8 expert weights in every mode; placement changes latency and capacity, not numerical precision.
\end{itemize}

\subsection{Implement expert-slot addressing only if required}
\begin{itemize}
    \item Keep dense attention tensors, routers, embeddings, norms, and codec output heads resident during output generation, and keep authoritative Q8 copies of routed experts in pinned CPU RAM.
    \item Derive the GPU slot-pool size from measured speaking-phase headroom and trace-derived expert working sets; do not reserve a fixed 1.5~GiB pool in advance.
...
\begin{enumerate}
    \item Freeze the PyTorch parity fixtures before implementing the native speech-output graphs.
    \item Verify MTP and Code2Wav independently, then require the complete frozen-input waveform chain to pass before reducing scope or schedule.
    ...
    \item The selected Talker residency mode passes fidelity and latency gates before cache-policy research; custom expert paging is required only if full residency and ordinary offload fail.
...
\begin{itemize}
    ...
    \item They may be revisited after the native runtime, Talker residency selection, fixed pooling, exact VoxZip reproduction, learned compactor, and bounded temporal retention are stable.
