...
The project can load the complete pinned Qwen3-Omni model on an A100 and execute Thinker, Talker, MTP, and Code2Wav. The successful probe placed Thinker layers 0–20, Talker, and Code2Wav on the A100; Thinker layers 21–47 remained on CPU. It produced one complete codec frame and a `[1, 1, 1365]` float32 waveform. The probe measured 36,511,910,400 bytes of peak GPU allocation. The stored evidence is the [A100 load probe](../../proposals/full_plan/8-23/runs/P0-003B/20260823T161735Z-a100/load_probe.json).
...
The [fixture harness](../../proposals/full_plan/8-23/tools/reference/fixture_harness.py) defines 154 boundary-and-case pairs that the PyTorch model can produce. The future native runtime owns eight scheduler and bounded-context pairs because PyTorch lacks the corresponding operations. The [fixture validator test](../../proposals/full_plan/8-23/tests/orchestration/test_fixture_validation.py) checks that partition.

The generator and `tests/fixtures/reference/manifest.json` remain absent. The lifecycle file labels fixture generation as `running`; [the lifecycle history](../../proposals/full_plan/8-23/orchestration/state.json) records only the start of the work packet. Colab reports zero active sessions. In operational terms, the probe finished and fixture capture awaits implementation.
...
CleaRx aims to run Qwen3-Omni as a native waveform-to-waveform system in one C++ process on a single 24 GB NVIDIA L4. The native runtime must preserve the pinned model's behavior while bounding live memory independently of conversation length. The [implementation proposal](../../proposals/full_plan/8-23/docs/proposal.tex) defines that target and hardware constraint.
...
- [Native Qwen3-Omni waveform runtime proposal](../../proposals/full_plan/8-23/docs/proposal.tex)
- [Reference fixture contract](../../proposals/full_plan/8-23/docs/architecture/fixture_contract.md)
- [Fixture harness](../../proposals/full_plan/8-23/tools/reference/fixture_harness.py)
- [Fixture-generation work packet](../../proposals/full_plan/8-23/orchestration/work_packets/P0-003B.yaml)
- [A100 load probe](../../proposals/full_plan/8-23/runs/P0-003B/20260823T161735Z-a100/load_probe.json)
- [Evaluation corpus manifest](../../proposals/full_plan/8-23/evaluation_corpus_manifest.json)
...
The native baseline supplies useful loading, backend, convolution, attention, and mixture-of-experts machinery. CleaRx still owns the complete Talker, MTP, Code2Wav, conversion, and scheduling path described in the [runtime gap matrix](../../proposals/full_plan/8-23/docs/architecture/runtime_gap_matrix.md).
...
The A100 defines what the pinned reference model computes. The L4 must later establish whether the native implementation fits and runs in real time. The [memory model](../../proposals/full_plan/8-23/docs/architecture/memory_model.md) remained a planning model on August 23 because every `measured_bytes` field was still empty.
...
The `commons-jfk-inaugural` corpus item provides the long Thinker case. The [evaluation corpus manifest](../../proposals/full_plan/8-23/evaluation_corpus_manifest.json) identifies the compressed recording, its SHA-256 digest, its 840.228345-second descriptive duration, its prompt, and a 384-token output cap.
...
The tensor inventory reconciled 28,010 tensors across 15 checkpoint shards. The fixture schema, comparison profiles, provenance joins, boundary matrix, and evidence-origin split were executable. The [A100 preflight](../../proposals/full_plan/8-23/runs/P0-003B/20260823T155524Z-a100/preflight.json) verified the pinned model and Transformers revisions, checkpoint access, 20 Talker layers, 15 residual codebooks, and 16 Code2Wav quantizers.

Automatic placement loaded the model and then encountered a meta-device tensor during generation. The explicit map placed Thinker layers 0–20, Talker, and Code2Wav on the A100 while Thinker layers 21–47 remained in CPU memory. The [saved A100 probe](../../proposals/full_plan/8-23/runs/P0-003B/20260823T161735Z-a100/load_probe.json) completed Thinker, Talker, MTP, and Code2Wav. It produced one codec frame and a `[1, 1, 1365]` float32 waveform in 26.692 seconds with 36,511,910,400 bytes of peak GPU allocation.
