...
## How this audit decides support
...
| Backend scheduling | [`ggml_backend_sched_new`](https://github.com/ggml-org/llama.cpp/blob/6657ded4faa3b8450221119fc6b4d002e35104a2/ggml/include/ggml-backend.h#L319) | GGML can assign one graph across registered backends. CleaRx must still decide which backend executes each phase and when buffers move. |
| Async expert transfer | [`ggml_backend_tensor_copy_async`](https://github.com/ggml-org/llama.cpp/blob/6657ded4faa3b8450221119fc6b4d002e35104a2/ggml/include/ggml-backend.h#L116) | This function can start the asynchronous tensor copy required by the later expert-cache spike. |
| Memory instrumentation | `ggml_backend_dev_memory`, `ggml_backend_sched_get_buffer_size`, `llama_model_size`, and `llama_perf_context` | The runtime exposes device free/total memory, scheduler buffer bytes, model bytes, and timing counters. P0-005 states which values to sample during each phase. |
