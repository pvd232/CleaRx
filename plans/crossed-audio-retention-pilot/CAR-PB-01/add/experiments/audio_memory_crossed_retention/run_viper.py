...
def materialize_audit_archive(root: Path, resolved_run: Path) -> tuple[bytes, str]:
    ...
    with tempfile.TemporaryDirectory(prefix="clearx-viper-audit-") as temporary:
        ...
        required = (
            Path("spec.yaml"),
            Path("variants/ravdess_crossed_equal_budget.spec.yaml"),
            ...
            / "stages/measure_retention/spec.yaml",
            run_root.relative_to(experiment_root)
            / "stages/measure_retention/resolved.yaml",
            run_root.relative_to(experiment_root) / "stages/verify_retention/spec.yaml",
            run_root.relative_to(experiment_root)
            / "stages/verify_retention/resolved.yaml",
            ...
            run_root.relative_to(experiment_root)
            / "attempts/1/invocations/measure_retention.yaml",
            run_root.relative_to(experiment_root)
            / "attempts/1/invocations/verify_retention.yaml",
            run_root.relative_to(experiment_root)
            / "attempts/1/logs/measure_retention.stdout.log",
            run_root.relative_to(experiment_root)
            / "attempts/1/logs/measure_retention.stderr.log",
            run_root.relative_to(experiment_root)
            / "attempts/1/logs/verify_retention.stdout.log",
            run_root.relative_to(experiment_root)
            / "attempts/1/logs/verify_retention.stderr.log",
            run_root.relative_to(experiment_root)
